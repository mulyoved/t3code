...
export function activateWeb(ctx) {
  ctx.composer.registerSource({
    id: "slash-commands",
    triggers: ["slash-command"],
    async getItems(input) {
      const [skillsResult, workspaceResult] = await Promise.all([
        ctx.callServer("skills.list", input.cwd ? { cwd: input.cwd } : undefined).catch(() => ({
          skills: [],
        })),
        input.cwd
          ? ctx
              .callServer("workspace.list", {
                cwd: input.cwd,
                query: "",
                limit: 40,
              })
  ...
  ctx.composer.registerSource({
    ...
    async getItems(input) {
      const result = await ctx.callServer("skills.list", input.cwd ? { cwd: input.cwd } : undefined);
  ...
  ctx.composer.registerSource({
    ...
    async getItems(input) {
      ...
      const result = await ctx.callServer("workspace.list", {
        cwd: input.cwd,
        query: input.query,
        limit: 120,
      });
